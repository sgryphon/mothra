...
namespace Example
{
    internal static class MothraInterop
    {
        ...
        public static extern unsafe int Start([In, Out] string[] args, int length);

        [DllImport(DllName, EntryPoint = "libp2p_send_gossip")]
        public static extern unsafe int SendGossip(byte* topicUtf8, int topicLength, byte* data, int dataLength);

        [DllImport(DllName, EntryPoint = "libp2p_send_rpc_request")]
        public static extern unsafe int SendRequest(byte* methodUtf8, int methodLength, byte* peerUtf8, int peerLength, byte* data, int dataLength);

        [DllImport(DllName, EntryPoint = "libp2p_send_rpc_response")]
        public static extern unsafe int SendResponse(byte* methodUtf8, int methodLength, byte* peerUtf8, int peerLength, byte* data, int dataLength);
